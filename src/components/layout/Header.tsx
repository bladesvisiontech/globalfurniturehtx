...
export default function Header() {
  ...
  return (
    <header className="bg-white sticky top-0 z-50" style={{ borderBottom: '1px solid #F0F0F0' }}>
      ...
      <div className="bg-[#0e2b62] text-white text-xs tracking-wide py-2">
      ...
      <div className="max-w-screen-xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex flex-col leading-none">
          <span className="text-[15px] font-semibold tracking-[0.18em] uppercase text-[#0e2b62]">
            Global Furniture
          </span>
          ...
          {NAV_LINKS.map((link) =>
            link.hasDropdown ? (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 text-sm text-[#0e2b62]/80 hover:text-[#0e2b62] transition-colors font-medium"
                >
                  {link.label}
                    ...
                    >
                      {SHOP_CATEGORIES.map((cat) => (
                        <Link
                          key={cat.href}
                          href={cat.href}
                          className="block px-4 py-2.5 text-sm text-[#0e2b62]/70 hover:text-[#0e2b62] hover:bg-[#F5F2EC] transition-colors"
                        >
                          {cat.label}
                        </Link>
                      ))}
              ...
              <Link
                key={link.href}
                href={link.href}
                className="text-sm text-[#0e2b62]/80 hover:text-[#0e2b62] transition-colors font-medium"
              >
                {link.label}
              </Link>
            )
        ...
        {/* Mobile hamburger */}
        <button
          className="lg:hidden p-2 text-[#0e2b62]/60"
          ...
          <nav className="px-6 py-4 flex flex-col gap-1">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className="py-3 text-sm font-medium text-[#0e2b62]/80 hover:text-[#0e2b62] border-b border-[#F0F0F0] last:border-0"
              >
                {link.label}
              </Link>
            ))}

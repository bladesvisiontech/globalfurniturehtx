...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <div>
      {/* Hero image */}
      {post.coverImage && (
        <div className="relative w-full bg-[#F5F2EC]" style={{ height: 480 }}>
          <Image
            src={post.coverImage}
            alt={post.title}
            fill
      ...
      <div className="max-w-2xl mx-auto px-6 py-16">
        ...
        {/* Content: paragraphs with images injected between them */}
        <div className="space-y-6">
          {paragraphs.map((para, idx) => {
            const isHeading = para.startsWith('**') && para.endsWith('**')
            const inlineImage = contentImages.find((img) => img.afterParagraph === idx)

            return (
              <div key={idx}>
                {isHeading
                  ? <h3 className="text-lg font-medium text-[#1E3331] mt-8">{para.replace(/\*\*/g, '')}</h3>
                  : <p className="text-[#1E3331]/60 font-light leading-relaxed text-sm">{para}</p>
                }

                {inlineImage && (
                  <figure className="mt-8 mb-2">
                    <div className="relative w-full bg-[#F5F2EC] overflow-hidden" style={{ height: 340 }}>
                      <Image
                        src={inlineImage.src}
                        alt={inlineImage.alt}
                        fill
                    ...
                    {inlineImage.caption && (
                      <figcaption className="text-[11px] text-[#1E3331]/35 font-light mt-2 text-center tracking-wide">
                        {inlineImage.caption}
                      </figcaption>
                    )}
                  </figure>

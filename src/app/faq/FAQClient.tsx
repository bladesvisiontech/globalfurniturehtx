  ...
  return (
    <div style={{ border: '1px solid #ECEAE4' }}>
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-6 py-4 text-left hover:bg-[#F5F2EC] transition-colors">
        <span className="text-sm font-medium text-[#1E3331] pr-4">{item.question}</span>
        <ChevronDown size={16} className={cn('text-[#7DA68B] shrink-0 transition-transform', open && 'rotate-180')} />
      </button>
      {open && (
        <div className="px-6 pb-5 text-[#1E3331]/60 text-sm font-light leading-relaxed" style={{ borderTop: '1px solid #ECEAE4', paddingTop: 16 }}>
          {item.answer}
        </div>
      )}
    </div>
...
export default function FAQClient({ grouped }: { grouped: Record<string, FAQItem[]> }) {
  return (
    <div className="max-w-3xl mx-auto px-6 py-16">
      ...
      <div className="bg-[#1E3331] text-white p-10 text-center mt-4">
        ...
        <Link href="/contact" className="text-sm font-medium text-[#1E3331] bg-white px-8 py-3 hover:bg-[#E7E0CE] transition-colors" style={{ borderRadius: 25 }}>Contact Us</Link>
      </div>
    </div>
  )
}
